...
public class SignAdapter extends RecyclerView.Adapter {
    ...
    public SignAdapter(List<SignBean.DataBean.SignTopBean.ListBean> list, Context context) {
        this.list = list;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        return new LocalAdapter.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {

        LocalAdapter.ViewHolder viewHolder= (LocalAdapter.ViewHolder) holder;
        viewHolder.tv_title.setText(list.get(position).getNickName());
        viewHolder.tv_experience.setText(list.get(position).getTongQian()+"铜钱");
